...
const SoulTraceAssessment: React.FC<SoulTraceAssessmentProps> = ({ onComplete, onCancel }) => {
  ...
  const fetchNextQuestion = async (currentAnswers: { questionId: number; score: number }[]) => {
    ...
    setError(null);
    try {
      const workerUrl = import.meta.env.VITE_WORKER_ANALYZE_FOOD || 'https://natofit.datomoliveira.workers.dev';
      const response = await fetch(`${workerUrl}/soultrace`, {
        method: 'POST',
      ...
      if (!response.ok) {
        ...
        console.error('Erro na resposta da API SoulTrace (via Proxy):', response.status, errorText);
      ...
      setError(err.message || 'Houve um problema ao carregar o questionário. Tente novamente em instantes.');
    } finally {
      setLoading(false);
    }
  };
